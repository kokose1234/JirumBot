...
{
    public class CoolService : SimpleCrawlManager
    {
        ...
        public CoolService(DiscordSocketClient discord, IOptions<BotSetting> config, LoggingService logging, UserRepository userRepository)
        {
            _discord = discord;
        ...
        protected override async Task<bool> FetchNewArticles()
        {
            ...
            try
            {
                var response = await _httpClient.GetStringAsync("https://coolenjoy.net/bbs/jirum");
                if (string.IsNullOrEmpty(response)) return false;
                _document.LoadHtml(response);

                var list = _document.DocumentNode.SelectNodes(_config.CoolBasePath);
            ...
            {
               _logging.GetExceptionLogger().Error(ex, "쿨엔조이 새로고침 중 오류 발생");
                return false;
            }
        }
    }
